...
## Planificación (parte I) {#planificacion}
...
| [13](#clase-13) | S7 | 24 oct | Joins. Import/export | [💻](#tu-turno-13-1) [💻](#tu-turno-13-2) |  [🐣](#caso-practico-13-1) [🐣](#caso-practico-13-2) |  |
:::
:::

---

## Planificación (parte II) {#planificacion-II}

::: column-screen-inset-right
::: {style="font-size:20px"}
|  CLASE | SEMANA | FECHAS | TOPIC | EJ. | WORKBOOK | ENTREGA | 
|:------:|:--------:|:--------:|:------:|:------:|:------:|:------:|:------:|:------:|:------:|
| [14-15-16](#clase-14) | S8 | 28 y 31 oct  | Concurso | | |  |
| [17-18](#clase-17) | S9 | 4-7 nov | Listas. Casos reales | [💻](#tu-turno-17-1) | [🐣](#caso-practico-17-1) [🐣](#caso-practico-17-2) |  |
| [18](#clase-18) | S9 | 7 nov | Casos reales | | [🐣](#caso-practico-18-1) [🐣](#caso-practico-18-2) [🐣](#caso-practico-18-3)|  |
| [19](#clase-19) | S10 | 11 nov | Entrega III | | | [🎯 25%](#clase-19) |
| [20](#clase-20) | S10 | 14 nov | Intro al dataviz | | |  |
| [21](#clase-21) | S11 | 18 nov | ggplot1 | | |  |
| [22](#clase-22) | S11 | 21 nov | ggplot2 | | |  |
| [23](#clase-23) | S12 | 25 nov | ggplot3 | | |  |
| [24](#clase-24) | S12 | 28 nov | ggplot4 | | |  |
| [25](#clase-25) | S13 | 2 dic | casos prácticos (factores) | | |  |
| [26](#clase-26) | S13 | 5 dic | inferencia | | |  |
| [27](#clase-27) | S14 | 9 dic | casos prácticos (r base también) | | |  |
| [28](#clase-28) | S14 | 12 dic | sql + arrow | | |  |
| [29](#clase-29) | S15 | 16 dic | ... | | |  |
| [30](#clase-30) | S15 | 19 dic | Entrega IV | | | [🎯 30%](#clase-30) |
:::
:::
...
## 🐣 Caso práctico III: el señor de los anillos {#caso-practico-11-3}
...
[**Summarise y group_by(). Contar y resumir.**]{style="color:#444442;"}
...
[**¿Cómo cruzar dos tablas? ¿Cómo importar y exportar?**]{style="color:#444442;"}
...
* [🐣 Workbook/caso práctico I: Beatles](#caso-practico-13-1)
...
Intenta responder a las preguntas planteadas en el [**workbook**](https://javieralvarezliebana.quarto.pub/joins-import-export/#caso-pr%C3%A1ctico-i-beatles-y-rolling-stones)
...
## 🐣 Caso práctico II: encuesta CIS {#caso-practico-13-2}

[📊 Datos](https://drive.google.com/drive/folders/1HV3kAAHr4iWKvH5spsKWUwAdzWSBS8pw?usp=sharing)
...
Intenta responder a las preguntas planteadas en el [**workbook**](https://javieralvarezliebana.quarto.pub/joins-import-export/#caso-pr%C3%A1ctico-iii-encuesta-cis)


# Clase 14-15-16: [concurso]{.flow} {#clase-14}
...
* un fichero `.qmd` creado donde vamos a realizar cada ejercicio.
...
# Clase 17-18: [listas]{.flow} {#clase-17}


[**Manejo de listas con paquete purrr. Ejemplos casos rales**]{style="color:#444442;"}


* [💻 Ejercicios resueltos: listas](#tu-turno-17-1) 

* [🐣 Workbook/caso práctico I: renta municipios](#caso-practico-17-1)

* [🐣 Workbook/caso práctico II: estadística descriptiva](#caso-practico-17-2)
...
Ya hemos visto que las listas son un objeto en R que nos permite almacenar [**colecciones de variables de distinto tipo**]{.hl-yellow} (como con `data.frame` y `tibble`) pero también [**diferentes longitudes**]{.hl-purple}, con estructuras totalmente heterogéneas (incluso una lista puede tener dentro otra lista).
...
También podemos hacer [**listas con otras listas dentro**]{.hl-yellow}, de forma que para acceder a cada nivel debamos utilizar el operador `[[]]`.
...
Sin embargo una de las desventajas es [**no puede ser vectorizada**]{.hl-yellow} inmediatamente, por lo que cualquier operación aritmética aplicada a una lista dará [**error**]{.hl-red}. Por ejemplo, en el código anterior, uno esperaría que hiciese la operación en cada elemento de la lista (en `a` y en `b`) pero no funciona.
...
Este paquete pretende imitar la [**programación funcional**]{.hl-yellow} de otros lenguajes como Scala o la [**estrategia map-reduce**]{.hl-yellow} de Hadoop (de Google).
...
La función más sencilla del paquete `{purrr}` es la función `map()`, que [**aplica una función vectorizada**]{.hl-yellow} a cada uno de los elementos de una lista. Veamos un primer ejemplo aplicado a vectores: imagina que tenemos la siguiente lista y queremos aplicar, a cada uno de sus elementos, la suma.
...
`map(lista, función)` nos permite [**mapear la lista**]{.hl-yellow} y aplicar la función que queramos elemento a elemento
...
Veamos otro ejemplo que puedes sernos útil: imagina que tenemos [**dos muestras aleatorias de distinto tamaño**]{.hl-yellow}. La única forma en la que podemos almacenarlas juntas es con una lista pero...¿cómo hacer la media a ambas distribuciones sino podemos vectorizarlas?
...
¿Y si quisiéramos calcular la media de sus valores al cuadrado? En este caso [**no disponemos de una función ya definida**]{.hl-red} así que tenemos dos opciones: definirla antes (con un nombre) o bien [**definir la función dentro del propio map**]{.hl-yellow}
...
Una forma de [**definir la función más rápido**]{.hl-yellow} es así:
...
Además de ser [**más legible y eficiente**]{.hl-yellow}, con `{purrr}` podemos [**decidir el formato de salida**]{.hl-yellow} tras la operación

* salida como un [**vector numérico (decimales)**]{.hl-purple} con `map_dbl()`
* salida como un [**vector numérico (enteros)**]{.hl-purple} con `map_int()`
* salida como un [**vector de caracteres**]{.hl-purple} con  `map_chr()`
* salida como un [**vector numérico lógico**]{.hl-purple} con  `map_lgl()`
...
También puede sernos útil para [**acceder a un elemento de CADA lista**]{.hl-yellow} si le pasas un [**número**]{.hl-yellow} en lugar de una función.
...
También podemos hacer uso de `pluck()` para [**acceder al i-th objeto de la lista**]{.hl-yellow}.
...
También tenemos la opción de **generalizarlo** para poder utilizar funciones que [**necesiten dos argumentos**]{.hl-yellow} (operaciones binarias) con `map2()`. Por ejemplo, supongamos que tenemos las siguientes dos listas, y queremos que suma el vector del primer elemento de ambas entre sí, y lo mismo con el segundo
...
Podemos también obtener la salida en forma de `data.frame` añadiendo `list_rbind()` o `list_cbind()`, que [**convierte una lista en una tabla**]{.hl-yellow}.
...
Podemos generalizarlo aún más con `pmap_xxx()` que nos permite utilizar [**múltiples argumentos (múltiples listas para funciónes multivariantes)**]{.hl-amarillo}.
...
Tenemos otros tipos de iteradores que, aunque asumen entradas, no devuelven nada, como `walk()` (sólo un argumento de entrada), `walk2()` (dos argumentos) y `pwalk()` (múltiples argumentos), todos [**devuelven algo invisible**]{.hl-yellow}, sólo llaman a una función por sus [**efectos secundarios**]{.hl-yellow} en lugar de por su valor de retorno.
...
## 🐣 Caso práctico I: renta municipios {#caso-practico-17-1}

En el archivo `municipios.csv` tenemos guardada la información de los municipios de España a fecha de 2019.

* La variable `LAU_code` representa el código como unidad administrativa local según la estandarización de la UE ([ver más](https://ec.europa.eu/eurostat/web/nuts/local-administrative-units)).
...
Intenta responder a las preguntas planteadas en el [**workbook**](https://javieralvarezliebana.quarto.pub/joins-import-export/#caso-pr%C3%A1ctico-ii-renta-municipios)

---

## 🐣 Caso práctico II: encuesta de satisfacción {#caso-practico-17-2}
...
También se puede hacer en `R base` con `table()` (que además de ser más sencillo respeta la jerarquía de la ordinal). [**Moraleja: a veces R base nos facilita la vida, no lo olvidemos porque lo vas a necesitar**]{.hl-yellow }
