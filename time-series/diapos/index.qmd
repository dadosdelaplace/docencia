...
Hasta ahora estamos usando siempre la [**misma metodología**]{.hl-yellow}

1. ¿Es mi proceso estocástico [**ruido blanco**]{.hl-green}? Un proceso de ruido blanco tiene a) **media constante 0**; b) **varianza constante**; c) **instantes incorrelados**.
...
2. Si no fuese ruido blanco, ¿es [**estacionario**]{.hl-green}? Un proceso estacionario tiene a) **media constante**; b) **varianza constante**; c) **correlaciones solo dependientes del lag $h$**.
...
3. Si es estacionario, ¿qué [**estructura ARMA (p, q)**]{.hl-green} tiene?
...
* [**Si ninguna de las dos se cumple**]{.hl-red} implica que tenemos una **estructura mixta ARMA(p, q)** con $p,q > 1$.
...
* En esta fase el objetivo [**no es obtener un modelo adecuado**]{.hl-red} a la primera ya que estabmos usando una muestra, sino [**restringir el conjunto de todos los posibles modelos ARMA**]{.hl-yellow} a un subconjunto más pequeño
...
La idea es [**empezar por el modelo más sencillo de los plausibles**]{.hl-yellow} para luego proceder a su [**diagnóstico**]{.hl-yellow}

. . .

4. [**Fase de diagnosis**]{.hl-yellow}: ¿es el modelo ajustado el más verosimil teniendo en cuenta que tenemos una muestra de dicha distribución?
...
El objetivo es [**chequear que las hipótesis planteadas se cumplen**]{.hl-green}
...
b) ¿Tiene el **residuo varianza constante**?
...
c) ¿Son los **residuos incorrelados en el tiempo**?
...
En definitiva, [**¿son nuestros residuos ruido blanco?**]{.hl-green}
...
También podemos [**comprobar si los residuos son normales**]{.hl-yellow}: si lo fuesen, sabríamos que [**incorrelación (lineal) implicaría independencia**]{.hl-green}: nada por modelizar, hemos acabado.
...
Un chequeo que puede ser también interesante es [**realizar una sobreparametrización del modelo**]{.hl-yellow}: si hemos ajustado un ARMA(p, q) y cumple las hipótesis, ¿funciona sustancialmente mejor un ARMA(p + 1, q) o un ARMA(p , q+ 1)?

. . .

[**Si la fase de diagnosis no fuese exitosa**]{.hl-red} deberíamos comenzar de nuevo el proceso ajustando un proceso de mayor orden
...
5. [**Fase de evaluación**]{.hl-yellow}: ¿cómo funciona nuestro modelo? ¿cuánto se equivoca?

Es aquí donde deberemos aplicar lo que conocemos respecto a [**validación de modelos**]{.hl-yellow}

a) separamos en **train y test**

b) si tenemos **varios posibles modelos paramétricos** candidatos y tenemos dudas de cuál funcionaría mejor (o de si un modelo más complejo es rentable), realizamos una **validación cruzada para obtener un métrica promedio**

c) El **modelo ganador lo volvemos a lanzar en train y test** para obtener las métricas finales.
...
Hasta ahora hemos supuesto que nuestro procesos eran estacionarios ya  quelos [**procesos ARMA(p,q) solo pueden ser aplicados a procesos estacionarios**]{.hl-yellow}.

. . .

Sin embargo una de las etapa más importantes es, en caso de que no lo sea, [**detectar la estructura no estacionaria**]{.hl-yellow} de mi proceso

* ¿Falla en la [**estacionariedad en varianza**]{.hl-red}?

* ¿Falla en la [**estacionariedad en media**]{.hl-red}?

---

## Procesos no estacionarios

Si nuestro proceso [**no fuera estacionario en varianza**]{.hl-red} deberíamos evaluar que [**tendencia tiene la variabilidad de la serie respecto a su nivel para transformar los datos**]{.hl-yellow}. Para ello tenemos dos opciones:
...
Si nuestro proceso [**no fuera estacionario en media**]{.hl-red} deberíamos evaluar que [**tendencia tiene la propia serie**]{.hl-yellow}. 

¿Cómo **hacer la serie estacionaria en media**?

. . .

La forma más sencilla es realizar lo que se conoce como [**diferenciaciones d ela serie**]{.hl-yellow}. Dado un proceso estocástico $X_t$ definiremos el [**operador diferencia de orden 1**]{.hl-yellow} como 

$$Y_t:= \nabla X_t = (1-B)X_t = X_t - X_{t-1}$$

. . .

Dado un proceso estocástico $X_t$ definiremos el [**operador diferencia de orden d**]{.hl-yellow} como 
...
Con dicho operador diremos que $X_t$ es un [**proceso autoregresivo integrado de medias moviles ARIMA(p, d, q)**]{.hl-yellow} si tras aplicar $d$ diferencias con $\nabla^d$, el proceso resultante es un **proceso ARMA(p, q) invertible y estacionario** y se define como
...
Para [**identificar el orden $d$**]{.hl-yellow} suele funcionar la regla de que $d=1$ elimina tendencias lineales y $d=2$ tendencias cuadráticas. También podemos visualizar la serie tras aplicar `difference()` o hacer uso del contraste `unitroot_ndiffs` que nos indica el orden $d$ que identifica en nuestro proceso (o dejar libre el parámetro en `ARIMA()` y que el ajuste automática decida)
...
Como hemos dicho un [**proceso mixto ARIMA(p, d, q)**]{.hl-yellow} es definido como

$$\left(1 - \phi_1 B - \ldots - \phi_p B^p \right) \left( 1 - B \right)^d X_t = \left(1 - \theta_1 B - \ldots - \theta_q B^q \right)  \varepsilon_t$$
que de manera compacta puede ser definido como 
...
Si además no fuese estacionario en varianza, amplicando una [**transformación Box-Cox**]{.hl-yellow}, tendríamos finalmente
...
Este enfoque es suficientemente flexible para [**modelizar procesos no estacionarios (pero que lo pueden ser tras diferenciar y aplicar una transformación)**]{.hl-yellow} pero sin estacionalidad. ¿Cómo incluirla?

. . .


Para entenderlo vamos a empezar con lo que se conoce como [**procesos estacionales puros**]{.hl-yellow}, procesos donde solo hay una componente estacional y nada más. Por ejemplo, pensemos en la ecuación de un AR(1)

$$X_t - \phi_1 X_{t-1} = (1 - \phi_1 B) X_t = \Phi_1(B)X_t = \varepsilon_t$$
¿Cómo hacerlo puramente estacional?
...
$$X_t - \phi_1 X_{t-1} = (1 - \phi_1 B) X_t = \Phi_1(B)X_t = \varepsilon_t$$
En un contexto **puramente estacional**, el **concepto «dato anterior»** no es el dato que corresponde al instante previo $t-1$ sino al **dato que corresponde al periodo estacional previo**. Si tenemos una estacionalidad de **periodo $s$** entonces [**retroceder estacionalmente 1 periodo es $X_{t-s}$**]{.hl-yellow}.

. . .

Por denotaremos como $AR(1)_s$ un [**proceso autorregresivo puramente estacional de periodo s y orden 1**]{.hl-yellow} al proceso
...
De la misma manera podremos definir un [**proceso autorregresivo puramente estacional de periodo s y orden P**]{.hl-yellow} al proceso
...
Y también podremos definir un [**proceso de medias móviles puramente estacional de periodo s y orden Q**]{.hl-yellow} al proceso
...
[**¿Cómo simularlos?**]{.hl-yellow} Para ello vamos a definir un $AR(1)_{s = 12}$ con $\widetilde{\phi}_1 = 0.8$. Para ello vamos a hacer uso del paquete `{astsa}`
...
* [**ACF**]{.hl-yellow}: en el caso de un AR(1) las correlaciones descendían a 0 asintóticamente; en el caso de un $AR(1)_s$ dicho [**comportamiento asintótico solo sucede en las correlaciones múltiplos de s**]{.hl-yellow}

* [**PACF**]{.hl-yellow}: en el caso de un AR(1) las correlaciones a partir de la primera caían a 0 drásticamente; en el caso de un $AR(1)_s$ [**decaen drásticamente a 0 pero a partir de la correlación $\rho_s$**]{.hl-yellow}.
...
* [**ACF**]{.hl-yellow}: en el caso de un AR(2) las correlaciones descendían a 0 asintóticamente; en el caso de un $AR(2)_s$ dicho 
[**comportamiento asintótico solo sucede en las correlaciones múltiplos de s**]{.hl-yellow}


* [**PACF**]{.hl-yellow}: en el caso de un AR(2) las correlaciones a partir de $\rho_{2}$ caían a 0 drásticamente; en el caso de un $AR(2)_s$ [**decaen drásticamente a 0 pero a partir de la correlación $\rho_{2*s}$**]{.hl-yellow}.
...
[**💻 Te toca**]{.hl-yellow}: simula distintos procesos $MA$ puramente estacionales y «adivina» la forma de sus ACF/PACF antes de comprobarlo

---

## Procesos SARIMA

Como pasa en los procesos ordinarios, en el caso de **procesos puramente estacionales** también podremos juntar ambas parte y definir un [**proceso mixto autorregresivo de medias móviles puramente estacional de periodo s**]{.hl-yellow} al proceso $ARMA(P, Q)_s$ definido como
...
$$ \widetilde{\Phi}_P(B^s) \left(1 -B^s \right)^{D}X_t= \widetilde{\Phi}_P(B^s) \nabla_{s}^{D}X_t =  \widetilde{\Theta}_Q(B^s) \varepsilon_t$$

. . .

Fíjate que ahora, si $D=1$, $\left(1 -B^s \right)^{D}X_t = X_t - X_{t-s}$ (es una [**diferencia estacional**]{.hl-yellow})
...
* [**ACF**]{.hl-yellow}: en el caso de un ARIMA(0, 2, 0) las correlaciones tenían tendencia sin decaer a 0; en el caso de un $SARIMA(0, 2, 0)_s$ dicho comportamiento sucede en general PERO sobre todo en las **correlaciones múltiplos de s**.
...
Para terminar, vamos a [**juntar ambas partes (ordinaria y estacional)**]{.hl-yellow}: definiremos un [**proceso mixto autorregresivo integrado de medias móviles con componentes estacionales de periodo s**]{.hl-yellow} al proceso $SARIMA(p, d, q) \times (P, D, Q)_s$ definido como
...
$$ \Phi_p(B) \widetilde{\Phi}_P(B^s) \nabla_{s}^D \nabla^d X_t= \Theta_q(B)  \widetilde{\Theta}_Q(B^s) \varepsilon_t$$
tal que aplicando $d$ diferencias ordinarias y $D$ estacionales tenemos un proceso $SARMA(p, q) \times (P, Q)_s$.
...
1. Comprobamos  [**si hay que aplicar alguna diferencia ordinaria **]{.hl-yellow}
...
2. Si hace falta, **aplicamos las diferencias ordinarias**, y tras ello comprobamos  [**si hay que aplicar alguna diferencia estacional**]{.hl-yellow}

```{r}
SARMA_213 |> 
  features(difference(X_t, 2), unitroot_nsdiffs, .period = 12)
```
...
fit <-
  SARMA_213 |> 
  model("SARIMA" =
          ARIMA(X_t ~ 0 + pdq(p_init = 1, d = 0:2, q_init = 1) +
                  PDQ(P_init = 1, D = 0:2, Q_init = 1,
                      period = 12),
                greedy = FALSE, ic = "bic"))
```
