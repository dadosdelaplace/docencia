...
3. **Paso 3**: ¿cómo deberíamos de transformar los datos para poder pintar todas las series a la vez? 
...
datos_tidy <-
  datos |>
  pivot_longer(cols = "sd_05":"sd_4",
               names_to = "sd", values_to = "X_t")
ggplot(datos_tidy) +
...
Hazte un borrador de cómo sería el código de ggplot para dibujarlo (distinguiendo por ejemplo cada serie de un color, y separada del resto en forma de cuadrícula) y piensa que necesitas
...
time_series_error <- function(n, t = 1:n, sd_vec = 1) {
  ...
  for (i in 1:length(sd_vec)) {
    ...
    datos_tidy <- 
      datos_tidy |>
      bind_rows(tibble("t" = 1:n, "sd" = glue::glue("sd_{sd_vec[i]}"),
...
# Clase 4: [tendencia y estacionalidad]{.flow} {#clase-4}
...
💻 Construye una función llamada `time_series_error` que [**simule una serie temporal solo con error**]{.hl-yellow}. Los argumentos deben ser el tamañom muestral `n` y la desviación típica `sd` que queramos, y debe permitir que en el argumento `sd` pueda haber un vector de desviaciones (simular varias series a la vez, pero ya colocadas en tidydata para poder dibujarlas luego)
...
💻 Haciendo uso de la función anterior, define una función `time_series_trend_error` que simule una serie temporal con tendencia constante y error. Piensa como diseñarla de manera que solo tenga tres argumentos: el tamaño muestral, la desviación del error y la constante. A continuación usa dicha función y dibuja la serie.

```{r}
#| code-fold: true
time_series_trend_error <- function(n = 1000, trend = 0, sd = 1) {
  
  datos <-
    time_series_error(n = n, sd = sd) |>
    mutate(X_t = X_t + trend)
...
set.seed(123)
datos <- time_series_trend_error(n = 1000, trend = 3, sd = 0.5)
...
💻 Generliza la función anterior de manera que simule una [**serie temporal con error y tendencia lineal**]{.hl-yellow}, pero con solo 3 argumentos (donde antes definíamos solo una constante ahora será un vector de coeficientes). Fíjate que la **línea de ajuste de ggplot es literal la estimación** que haríamos si solo consideramos error + tendencia.


```{r}
#| code-fold: true
time_series_trend_error <- function(n = 1000, trend = c(1, -0.01), sd = 1) {
...
set.seed(123)
datos <- time_series_trend_error(n = 1000, trend = c(1, -0.01), sd = 0.5)
ggplot(datos, aes(x = t, y = X_t)) +
  geom_line(alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  ggthemes::scale_color_colorblind() +
  theme_minimal() +
  labs(x = "t", y = "X_t",
       title = "Serie temporal X_t = mu_t + eps_t con mu_t = 1 - 0.01*t")
```
...
💻 Generaliza la función anterior de manera que simule una [**serie temporal con error y tendencia polinómica**]{.hl-yellow} (que acepte un vector de parámetros general y construya el polinomio). 

```{r}
#| code-fold: true
time_series_trend_error <- function(n = 1000, trend = c(1, -0.01, 0.001, -0.0001), sd = 1) {
  
  datos <- time_series_error(n = n, sd = sd)
  for (i in 1:length(trend)) {
    
    datos <-
      datos |>
      mutate(X_t = X_t + trend[i]*t^(i - 1))
...
set.seed(123)
datos <- time_series_trend_error(n = 1000, trend = c(1, 0.01, 0.000001, -0.00000001), sd = 0.5)
ggplot(datos, aes(x = t, y = X_t)) +
  geom_line(alpha = 0.7) +
...
estim_ts_trend_error <- function(datos, degree = 1) {
  ...
  datos <-
    datos |> 
    mutate(X_hat = predict(modelo, tibble("t" = t)))
...
set.seed(123)
datos <-
  time_series_trend_error(n = 1000, trend = c(1, 0.01, 0.000001, -0.00000001), sd = 0.5)

# ajustamos un modelo polinómico con lm
...
retiro_estim <- 
  retiro |>
  select(fecha, tmed) |> rename(X_t = tmed) |> 
  mutate(t = 1:length(fecha)) |>
  estim_ts_trend_error(degree = 0) |> rename(tmed_hat_0 = X_hat) |>
  estim_ts_trend_error(degree = 1) |> rename(tmed_hat_1 = X_hat) |> 
  estim_ts_trend_error(degree = 3) |> rename(tmed_hat_3 = X_hat) |> 
  rename(tmed = X_t)

ggplot(retiro_estim |> pivot_longer(-c(fecha, t), names_to = "type", values_to = "pred")) +
...
Diremos que una serie tiene una [**componente estacional**]{.hl-yellow} siempre que presente un [**patrón que se repite en periodos (aprox.) fijos en el tiempo**]{.hl-yellow} tal que ahora
...
```{r}
datos <- tibble("t" = 1:15,
                "x" = c(0.8, 1.3, 1.6, 1.5, 2.2, 2.3, 2.2, 2.4, 2, 1.5, 1.2, 1.3, 1.2, 1, 0.7))
```
...
Para calcular una [**media móvil de orden $k=2*q+1$**]{.hl-yellow} en `R` podemos hacerlo con `filter()` del paquete `{stats}` ([**¡cuidado!**]{.hl-red}: si tienes cargado `{tidyverse}` debes especificar que `filter()` es)
...
datos <-
  datos |> 
  mutate(x_linear = predict(datos |> lm(formula = x ~ t), datos),
         x_smooth_3ma = stats::filter(x, filter = rep(1/k, k)))
datos
```
...
datos <-
  datos |> 
  mutate(x_smooth_5ma = stats::filter(x, filter = rep(1/5, 5)),
         x_smooth_7ma = stats::filter(x, filter = rep(1/7, 7)),
         x_smooth_9ma = stats::filter(x, filter = rep(1/9, 9)))
```
...
💻 Aplica el suavizado de medias móviles con $k=7, 14, 28, 365$ a los datos AEMET para incluir 4 nuevas columnas con los 4 métodos de estimación 
...
retiro_estim <-
  retiro_estim |> 
  mutate(x_smooth_7ma = stats::filter(tmed, filter = rep(1/7, 7)),
         x_smooth_14ma = stats::filter(tmed, filter = rep(1/14, 14)),
         x_smooth_28ma = stats::filter(tmed, filter = rep(1/28, 28)),
         x_smooth_365ma = stats::filter(tmed, filter = rep(1/365, 365)))
...
Dado que la mayoría de veces la serie temporal [**no se va a poder definir mediante una tendencia fija**]{.hl-red} (aunque dependa de $t$, los coeficientes de los que dependen no varian en el tiempo), una de las alternativas es lo que se conoce como [**regresión local LOESS o LOWESS**]{.hl-yellow}.

. . .

La idea de un ajuste local es [**ajustar a los datos una regresión polinómica (ponderada en el caso de LOWESS)**]{.hl-yellow}, de manera en cada punto el ajuste se hace  utilizando una metodología de [**mínimos cuadrados ponderado**]{.hl-yellow} dando **más peso a los puntos cercanos al punto cuya respuesta está siendo estimando y menos peso a los puntos más lejanos**.

```{r}
estim_ts_trend_error <- function(datos, degree = 1, span = NULL) {
  ...
  if (is.null(span)) {
    ...
    modelo <- datos |> loess(formula = X_t ~ t, degree = degree, span = span)
...
siguiente clase: STL y multiplicativo
...
1/10/2024: repaso todo en R

---

4/10: ¿a mano? ejercicios
...
11/10: teoría procesos estocásticos
15/10

---

5-6 clases de box-jenkins
