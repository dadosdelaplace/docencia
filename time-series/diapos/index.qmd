...
[**💻 Diseña una función**]{.hl-yellow} para simular $n$ trayectorias de un $ARMA(p, q)$ en función de $p$, $q$, $n$, $\sigma$, $\left(\phi_1, \ldots, \phi_p\right)$,  $\left(X_1, \ldots, X_{max(p,q)}\right)$, $\left(\theta_1, \ldots,\theta_q\right)$,  $\left(\varepsilon_1, \ldots, \varepsilon_q\right)$

```{r}
#| code-fold: true
ARMA_p_q_simul <- function(p, q, n, sigma, phi, theta, X, eps) {
  
  X_t <- X
  eps <- c(eps, rnorm(n - q, mean = 0, sd = sigma))
  for (i in (max(p, q) + 1):n) {
    X_t[i] <-
      sum(phi * X_t[(i - 1):(i-p)]) +
      rnorm(1, mean = 0, sd = sigma) -
      sum(theta * eps[(i - 1):(i-q)]) 
  } 
  ts <- tibble("t" = 1:n, "X_t" = X_t) |> as_tsibble(index = t)
  return(ts)
}
...
set.seed(1234567)
ARMA_2_1 <- 
  ARMA_p_q_simul(p = 2, q = 1, n = 10000, sigma = 1.5,
                 phi = c(-0.2, 0.7), theta = 0.8, 
                 X = rnorm(n = 2, mean = 0, sd = 1.5), eps = 0)
ggplot(ARMA_2_1) +
  geom_line(aes(x = t, y = X_t)) +
  theme_minimal()
```
...
También podemos hacer uso a partir de ahora de la [**función `arima.sim()` para simular procesos ARMA**]{.hl-yellow}
