...
¿Te has fijado en que [**cuando $\theta_1 = -1.5$ la serie explota**]{.hl-yellow}?
...
[**¿Por qué en función de $\theta_1$ el proceso es explosivo?**]{.hl-red}
...
Esta [**condición de invertibilidad**]{.hl-yellow} será fundamental en los procesos MA ya que, aunque la función de autocorrelación será importante, tenemos un [**problema de identificación**]{.hl-red}: dada una función de autocorrelación [**no existe un único proceso asociado**]{.hl-red}
...
¿Qué [**condiciones de invertibilidad**]{.hl-yellow} necesitamos?
...
```{r}
#| code-fold: true
MA_q_simul <- function(q, n, sigma, theta, eps_0) {
  
  X_t <- eps_0
  eps <- c(eps_0, rnorm(n = n - q, mean = 0, sd = sigma))
  for (i in (q + 1):n) {
...
💻 [**Aplica dicha función**]{.hl-yellow} para simular un $MA(2)$ con $n = 300$, $\sigma = 3$, $\left(\varepsilon_1, \varepsilon_2 \right) = 0$ y $\left(\theta_1, ~ \theta_2 \right) = (-0.3, 0.5)$


```{r}
#| code-fold: true
set.seed(1234567)
ggplot(MA_q_simul(2, 300, 3, theta = c(-0.3, 0.5), eps = rep(0, 2))) +
  geom_line(aes(x = t, y = X_t)) +
  labs(title = "MA(2) con theta_1 = -0.3, theta_2 = 0.5 y sigma = 3") +
  theme_minimal()
```

El proceso tiene que cumplir las condiciones de invertibilidad ya que parece estacionario. ¿Cuánto valen sus raíces?
...
Las [**raíces pueden ser complejas, lo importante es que su módulo sea mayor que 1**]{.hl-yellow}
...
💻 [**Aplica dicha función**]{.hl-yellow} para simular un $MA(3)$ con $n = 300$, $\sigma = 3$, $\left(\varepsilon_1,~\varepsilon_2, ~\varepsilon_3 \right) = 0$ y $\left(\theta_1, ~ \theta_2, ~\theta_3 \right) = (0.8, 0.4, -0.9)$


```{r}
#| code-fold: true
set.seed(1234567)
ggplot(MA_q_simul(3, 300, 3, theta = c(0.8, 0.4, -0.9), eps = rep(0, 3))) +
  geom_line(aes(x = t, y = X_t)) +
  labs(title = "MA(2) con theta_1 = 0.8, theta_2 = 0.4, theta_3 = -0.9 y sigma = 3") +
  theme_minimal()
```

¿Por qué es explosivo?

. . .

```{r}
Mod(pracma::roots(c(-0.9, 0.4, 0.8, 1)))
```
...
#| code-fold: true
set.seed(1234567)
MA_q_simul(2, 300, 3, theta = c(-0.3, 0.5), eps = rep(0, 2)) |> 
  ACF(X_t) |> 
  autoplot() +
  labs(title = "MA(2) con theta_1 = -0.3, theta_2 = 0.5 y sigma = 3") +
  theme_minimal()
...
#| code-fold: true
set.seed(1234567)
MA_q_simul(3, 300, 3, theta = c(0.8, 0.4, -0.9), eps = rep(0, 3)) |> 
  ACF(X_t) |> 
  autoplot() +
  labs(title = "MA(3) con theta_1 = 0.8, theta_2 = 0.4, theta_3 = -0.9 y sigma = 3") +
  theme_minimal()
```
...
💻 [**Aplica la función MA_q**]{.hl-yellow} para simular un $MA(2)$ con $n = 300$, $\sigma = 3$, $\left(\varepsilon_1, \varepsilon_2 \right) = 0$ y $\left(\theta_1, ~ \theta_2 \right) = (-0.3, 0.5)$. Compara ambos procesos (**mismos parámetros**)
...
De hecho en ambos casos al [**realizar el contraste de ruido blanco**]{.hl-yellow} de Ljung-Box obtenemos p-valores menores que 0.05 ==> [**rechazamos la hipótesis nula de ruido blanco**]{.hl-green}
...
Algo que no pasa si realmente tenemos ruido blanco.

```{r}
set.seed(1234567)
tibble("t" = 1:300, "X_t" = rnorm(n = 300, mean = 0, sd = 3)) |> 
  as_tsibble(index = t) |> 
  features(X_t, ljung_box)
```
...
```{r}
set.seed(1234567)
MA_q_simul(q = 1, n = 5000, sigma = 1, theta = c(0.75), eps = c(0)) |> 
  ACF(X_t, lag_max = 50)  |> 
  autoplot() +
...
```{r}
set.seed(1234567)
AR_p_simul(p = 1, n = 5000, sigma = 1, phi = c(0.75), X = c(0)) |> 
  ACF(X_t, lag_max = 50)  |> 
  autoplot() +
...
$$X_t = \phi_1 X_{t-1}  + \phi_2 X_{t-2} + \ldots + \phi_p X_{t-p}  + \varepsilon_t, \quad \left\lbrace \varepsilon_{t} \right\rbrace \text{ ruido blanco}$$
...
y se demuestra que $\alpha_p = \phi_p$ y $\alpha_h = 0$ para $h > p$.
...
```{r}
set.seed(12345)
MA_q_simul(q = 2, n = 300, sigma = 3, theta = c(-0.3, 0.4), eps = rnorm(n = 2, sd = 3)) |>
  PACF(X_t, lag_max = 40)  |> 
  autoplot() +
  labs(title = "MA(1) con theta_1 = -0.3, theta_2 = 0.4 y sigma = 3") +
  theme_minimal()
```
...
```{r}
set.seed(12345)
AR_p_simul(p = 2, n = 300, sigma = 3, phi = c(-0.3, 0.4), X = rnorm(n = 2, sd = 3)) |>
  PACF(X_t, lag_max = 40)  |> 
  autoplot() +
  labs(title = "AR(1) con phi_1 = -0.3, phi_2 = 0.4 y sigma = 3") +
  theme_minimal()
```
...
* En el caso de los procesos $MA(q)$: las [**autocorrelaciones parciales caen exponencialmente sin patrón**]{.hl-yellow}

* En el caso de los procesos $AR(p)$: las [**autocorrelaciones son aprox 0 a partir de $p$ y la correlación correspondiente a $p$ se sale de manera evidente**]{.hl-yellow}
...
Lo primero que haremos será generar un proceso $AR(2)$ y **dividir en train y test**

```{r}
set.seed(1234567)
datos <-
  AR_p_simul(p = 2, n = 300, sigma = 1,
             phi = c(-0.3, 0.5), X = c(0, 0)) 

train <- datos |> slice(1:280)
test <- datos |> slice(281:300)
```
...
predicciones <- fit_arima |> forecast(h = 20)
```
...
Tanto en train como en test el [**mejor modelo es el AR(2)**]{.hl-yellow} (lógico ya que los datos los hemos generado así)
...
Vamos entonces a [**repetir el proceso solo con dicho modelo**]{.hl-yellow}. Además con `report()` vamos a poder obtener las [**estimación de los coeficientes**]{.hl-yellow} ($-0.3198$ y $0.4557$, bastante preciso)

```{r}
fit_arima <- 
  train |> 
  model("AR2" = ARIMA(X_t ~ 0 + pdq(2, 0, 0)))
estimaciones <- fit_arima |> augment()
predicciones <- fit_arima |> forecast(h = 20)
report(fit_arima)
```
...
# Clases 22: [procesos SARIMA]{.flow} {#clase-22}
