...
Para calcularlo vamos a usar `features(variable, features = ...)` del paquete `{fabletools}` (ya cargado en `{fable}`). En `features = ...` vamos a especificar la opción `guerrero` (un método para elegir el $\lambda$ óptimo)
...
Todo tuyo a partir de aquí:

* Separa la muestra de momento solo en train-test

* Realiza todos los métodos de alisado conocidos así como otros modelos que sepas de fable PERO piensa y razona antes de ver su resultado cómo crees que van a funcionar y por qué

* Evalúalos en train-test y realiza el diagnóstico de los residuos.

* Usa el mejor de los métodos posibles en cada datasets pero ahora ajústalo con distintos valores de los parámetros $\alpha$, $\beta$ y $\gamma$ (cada parámetro del alisado) y decide los 3 mejores modelos haciendo uso de la validación cruzada para practicarla.
...
# Clases 16-17-18-19: [procesos estocásticos y arima]{.flow} {#clase-16}
