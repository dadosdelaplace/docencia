...
# Clase 9: [métodos de alisado]{.flow} {#clase-9}
...
La idea que tuvo **Holt (1956)** fue la de hacer una [**media ponderada**]{.hl-yellow} del valor real $X_t$ y su estimación $\widehat{X}_t$

$$\widehat{X}_{t+1} = \theta \widehat{X}_t + \left( 1 - \theta \right)X_t = X_t + \theta \left(\widehat{X}_t - X_t \right), \quad 0 < \theta < 1$$

. . .

* Si $\theta \to 1$, entonces $\widehat{X}_{t+1} \to \widehat{X}_t$ el [**modelo de alisado produce predicciones casi constantes**]{.hl-yellow} sin muchas variaciones

* Si $\theta \to 0$, entonces $\widehat{X}_{t+1} \to X_t$ el [**modelo de alisado produce predicciones muy variables dependientes del último valor observado**]{.hl-yellow} (con mucha fluctuación causada por la propia aleatoriedad de la serie)
...
$$\begin{eqnarray}\widehat{X}_{t+1} &=& \theta \left( \theta \widehat{X}_{t-1} + \left( 1 - \theta \right)X_{t-1} \right) + \left( 1 - \theta \right)X_t \nonumber \\ &=& \theta^2 \widehat{X}_{t-1} +  \left( 1 - \theta \right) \left(X_t + \theta X_{t-1} \right)\end{eqnarray}$$

. . .

Si repetimos el [**proceso iterativo**]{.hl-yellow} con $\widehat{X}_{t-1}$, $\widehat{X}_{t-2}$...

$$\begin{eqnarray}\widehat{X}_{t+1} &=& \theta^2 \left(\theta \widehat{X}_{t-2} + \left( 1 - \theta \right)X_{t-2} \right) +  \left( 1 - \theta \right) \left(X_t + \theta X_{t-1} \right) \nonumber \\ &=& \theta^3 \widehat{X}_{t-2} + \left( 1 - \theta \right) \left(X_t + \theta X_{t-1}  + \theta^2 X_{t-2}\right) \nonumber \\ &=& \theta^4 \widehat{X}_{t-3} + \left( 1 - \theta \right) \left(X_t + \theta X_{t-1}  + \theta^2 X_{t-2} + \theta^3 X_{t-3}\right) \nonumber \\ &=& ... \nonumber \\ &=& \theta^t \widehat{X}_{1} + \left( 1 - \theta \right) \left(X_t + \theta X_{t-1}  + \theta^2 X_{t-2} + \theta^3 X_{t-3} + \ldots + \theta^{t-1} X_{1}\right)  \end{eqnarray}$$

---

## Alisado simple

$$\widehat{X}_{t+1} = \theta^t \widehat{X}_{1} + \left( 1 - \theta \right) \left(X_t + \theta X_{t-1}  + \theta^2 X_{t-2} +  \ldots + \theta^{t-1} X_{1}\right)$$

¿Qué sucede cuando $t \to \infty$? 

. . . 

Dado que $\theta < 1$, entonces $\lim_{t \to \infty} \widehat{X}_{t+1} = \left( 1 - \theta \right) \sum_{j=0}^{\infty} \theta^j X_{t-j}$. 
...
Dado que $\theta < 1$, tenemos que

$$\sum_{j=0}^{\infty} \theta^j = \frac{1}{1 - \theta} \rightarrow \left( 1 - \theta \right) \sum_{j=0}^{\infty} \theta^j = 1$$

así que en realidad $\left( 1 - \theta \right) \sum_{j=0}^{\infty} \theta^j X_{t-j}$ representa una [**media ponderada del pasado (con pesos decrecientes geométricamente)**]{.hl-yellow} donde los pesos suman uno.
...
Si te fijas además [**todas las observaciones son influyentes**]{.hl-yellow} (en particular la primera) pero su influencia va decreciendo. 
...
$$X_t = T_t + \varepsilon_t, \quad {\rm E} \left[ T_t \right] \neq cte~ \forall t, \quad {\rm E} \left[ T_{t_i} \right] = cte ~\forall t \in t_i$$


una tendencia que va variando [**lentamente con el tiempo**]{.hl-yellow} (ya que el futuro se parece más a los valores pasados inmediatos que a los valores más lejanos).
...
Supondremos $\theta$ constante pero existen métodos (ver [aquí](https://robjhyndman.com/forecasting/)) para considerar que $\theta$ se adapta a su vez a lo largo del tiempo

---

## Datos sin estacionalidad ni tendencia
...
Como hemos visto el método anterior de alisado es bajo la hipótesis de que nuestros datos son localmente constantes, para datos sin tendencia
...
# Clase 10: [métodos de alisado]{.flow} {#clase-9}


---

# Clase 11: [teoría procesos]{.flow} {#clase-10}
