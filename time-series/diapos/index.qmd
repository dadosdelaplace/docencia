...
```{r}
datos$x_smooth_5ma <- stats::filter(x, filter = rep(1/5, 5))
datos$x_smooth_7ma <- stats::filter(x, filter = rep(1/7, 7))
datos$x_smooth_9ma <- stats::filter(x, filter = rep(1/9, 9))
```
...
💻 Diseña una función `estim_ts_smooth_ma()` que, imitando la función `estim_ts_trend_error()`, nos devuelva la estimación de una serie aplicando el suavizado de medias móviles con parámetro $k$. Importante: la serie que le pases a `stats::filter()` no puede tener ausentes.

. . .

```{r}
estim_ts_smooth_ma <-
  function(datos, k = 3, tag_estim = paste0("estim_ma_", k),
           col_t = "t", col_X_t = "X_t", nuevos_valores = NULL) {
    ...
    names(datos)[names(datos) == col_t] <- "t"
    names(datos)[names(datos) == col_X_t] <- "X_t"
    datos <- datos[order(datos$t), ] # ordenamos por t

    # eliminamos ausentes
    datos <- datos[!is.na(datos$X_t), ]
    
    # Estimamos
    datos[, tag_estim] <- stats::filter(datos$X_t, filter = rep(1/k, k))
...
💻 Diseña una función general `estim_ts()` donde con un argumento `tipo_estim` dentro llame a `estim_ts_trend_error()` o a `estim_ts_smooth_ma()` en función de lo que queramos. Piensa que esta función grande la opción de dar los mismos argumentos que las funciones a las que va a llamar dentro.
...
el suavizado de medias móviles con $k=7, 14, 28, 365$ a los datos AEMET para incluir 4 nuevas columnas con los 4 métodos de estimación 
