...
# Clases 17: [procesos estacionarios]{.flow} {#clase-17}
...
$$X_t = \mu + \sum_{j = -\infty}^{j=\infty} \Psi_j \varepsilon_{t-j}, \quad \sum_{j = -\infty}^{j=\infty} \left| \Psi_j \right| < \infty, \quad \left\lbrace \varepsilon_{t} \right\rbrace \text{ ruido blanco}$$
Si te fijas hay una constante inicial $\mu$ que representa la media (a largo plazo) del proceso ya que, dado que la esperanza es lineal y $\left\lbrace \varepsilon_{t} \right\rbrace$ es ruido blanco, $E \left[ X_t \right] = \mu + \sum_{j = -\infty}^{j=\infty} \Psi_j E \left[ \varepsilon_{t-j} \right] = \mu$

. . .

De ahora en adelante asumiremos que $\mu = 0$ ya que sino lo fuese basta con usar $X_t = X_t - \mu$

---

## Procesos MA

$$X_t = \sum_{j = -\infty}^{j=\infty} \Psi_j \varepsilon_{t-j}, \quad \sum_{j = -\infty}^{j=\infty} \left| \Psi_j \right| < \infty, \quad \left\lbrace \varepsilon_{t} \right\rbrace \text{ ruido blanco}$$
...
donde $q$ será el **orden** (lo llamaremos [**procesos de medias moviles de orden q o MA(q)**]{.hl-yellow}). Si te fijas es un **caso particular de proceso lineal** donde todos los coeficientes son 0 salvo $\left(-\theta_1,\ldots, -\theta_q \right)$.
...
$$B^s(X_t) = B^{s-1} (X_{t-1}) =  B^{s-1} \left( B X_t \right)  = B^{s-2} \left(  X_{t-2} \right)  = ... = X_{t-s}$$
...
El proceso anteriormente como $MA(q)$ se puede por tanto [**expresar en función del operador de retardos**]{.hl-yellow} como
...
$$\begin{eqnarray}\Theta_{q} \left(B \right) &=& \displaystyle \sum_{j=0}^{\infty} \Psi_j B^{j} \nonumber \\ \Psi_0 &=& 1, \nonumber \\ \Psi_j &=& -\theta_j, \quad j =1, \ldots,q,\nonumber \\ \Psi_j &=& 0, \quad j > q\end{eqnarray}$$

Dicho polinomio se le conoce como el [**polinomio de retardos o filtro lineal de retardos de orden $q$**]{.hl-yellow}

---

## Proceso MA

$$\begin{eqnarray}X_t &=& \varepsilon_t - \theta_1 \varepsilon_{t-1} - \ldots - \theta_q \varepsilon_{t-q}  = \varepsilon_{t} - \theta_1 B\varepsilon_t - \ldots - \theta_q B^{q}\varepsilon_t \nonumber \\ &=& \left( 1 - \theta_1 B - \ldots - \theta_q B^{q} \right)\varepsilon_t = \Theta_q \left(B \right)\varepsilon_t, \quad \left\lbrace \varepsilon_{t} \right\rbrace \text{ ruido blanco} \nonumber \end{eqnarray}$$

Dale valores $q=1, 2, 3$ y escribe cómo quedaría el proceso
...
$$X_t = \left( 1 - \theta_1 B - \ldots - \theta_q B^{q} \right)\varepsilon_t = \Theta_q \left(B \right)\varepsilon_t, \quad \left\lbrace \varepsilon_{t} \right\rbrace \text{ ruido blanco} \nonumber$$
...
$$E\left[X_t \right] = \left( 1 - \theta_1  - \ldots - \theta_q \right)E\left[ \varepsilon_{t} \right] = 0  \quad \text{(o bien } \mu \text{ si no está centrado)}$$
...
2. [**Varianza**]{.hl-yellow}:

$$Var\left[X_t \right] = \left(1 + \theta_{1}^{2} + \ldots + \theta_{q}^2  \right) \sigma_{\varepsilon}^2 = cte \quad \text{¿pero finita?}$$
...
* $h = 0$ -> $\gamma_0 = \sigma^2 + \theta_{1}^2 \sigma^2 = \left(1 + \theta_{1}^2 \right) \sigma^2$
...
Fíjate que el [**signo de $\rho_h$ depende del signo de $\theta_1$**]{.hl-yellow} y que a partir de $h$ las autocorrelaaciones son 0 (se dice que es [**q-correlacionado**]{.hl-yellow})
...
Si te fijas tenemos que $\varepsilon_t = X_t + \theta_1 \varepsilon_{t-1}$ por lo que sustituyendo tenemos que

$$\begin{eqnarray}X_t &=& \varepsilon_t + \theta_1 \left( X_{t-1} + \theta_1 \varepsilon_{t-2} \right) = \varepsilon_t + \theta_1  X_{t-1} + \theta_{1}^2 \varepsilon_{t-2} \nonumber \\ &=& \varepsilon_t + \theta_1  X_{t-1} + \theta_{1}^2 X_{t-1} + \theta_{1}^{3} \varepsilon_{t-3}\nonumber \\ &=& \varepsilon_{t} + \sum_{j=0}^{\infty} \theta_{1}^{j} X_{t-j}\end{eqnarray}$$

. . .

Parece lógico [**asumir que los valores más cercanos influyan más que los valores más retardados**]{.hl-yellow} por lo que $\left| \theta_{1}\right|^{j}$ debe disminuir con el retardo, es decir, [**necesitamos que $\left| \theta_{1}\right| < 1$**]{.hl-yellow}
...
Dicha condición $\left| \theta_{1}\right| < 1$ se la conoce como [**condición de invertibilidad**]{.hl-yellow} ya que nos permite expresar los errores en función de $X_t$ tal que
...
MA_1_simul <- function(n, sigma, theta_1, eps_1) {
  ...
  X_t <- eps_1
  for (i in 2:n) {
    X_t[i] <- rnorm(1, mean = 0, sd = sigma) - theta_1*X_t[i - 1]
  }
...
Aplícala para $n = 300$, $\sigma = 3$, $\varepsilon_1 = 0$ y $\theta_1 = (-0.3, 1.5)$
...
El primer ejemplo simulado que se cumplían las **condiciones de intertibilidad**, [**¿qué forma tiene su función de autocorrelaciones?**]{.hl-yellow}
...
Usaremos la [**función de autocorrelaciones (ACF)**]{.hl-yellow} para determinar si se trata de un [**proceso MA(1): si a partir de la primera las correlaciones caen drásticamente**]{.hl-yellow} (q-correlacionado)
...
Recuerda que estamos visualizando las [**estimaciones muestrales**]{.hl-yellow} por lo que cuanto más aumente $\theta_1$ más lenta será la caída.
