...
Normalmente una serie temporal suele ser más complejo y llevo al menos incorporada una [**componente de tendencia o nivel $\mu_t$**]{.hl-yellow} tal que
...
Dicha tendencia $\mu_t$ puede ser a su vez modelada en función de $t$ y de un vector de parámetros $\beta$ tal que $\mu_t := f \left(t, \beta \right)$
...
Para la predicción de los valores en un tiempo futuro $t + k$ simplemente necesitamos realizar la [**estimación del vector de parámetros $\widehat{\beta}$**]{.hl-yellow}. Para ello recurriremos al [**método de los minimos cuadrados**]{.hl-yellow} tal que
...
💻 Construye una función que [**simule una serie temporal con error y tendencia constante**]{.hl-yellow}. Usa la función definida anteriormente.
...
💻 Construye una función que simule una [**serie temporal con error y tendencia lineal**]{.hl-yellow}. Fíjate que la **línea de ajuste de ggplot es literal la estimación** que haríamos si solo consideramos error+tendencia.
...
💻 Construye una función general que simule una [**serie temporal con error y tendencia polinómica**]{.hl-yellow} (que acepte un vector de parámetros general y construya el polinomio). 
...
💻 ¿Se te ocurre alguna forma sencilla en `R` de obtener las estimaciones mínimos cuadrados asumiendo dicho modelo de tendencia + error?
...
modelo <- datos |> lm(formula = X_t ~ poly(t, 3, raw = TRUE))
trend_hat <- coef(modelo)
datos <-
  datos |> mutate(X_hat = predict(modelo, tibble("t" = t)))
ggplot(datos) +
  geom_line(aes(x = t, y = X_t), alpha = 0.7) +
...
💻 Crea una función que, **dada una serie, pueda incluir 3 nuevas columnas con los 3 métodos de estimación (tendencia constante, lineal y polinómica de grado 3)**. Aplica a dichos datos reales del AEMET
