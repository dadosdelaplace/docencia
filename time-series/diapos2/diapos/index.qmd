...
fit_algeria <-
  algeria_economy |>
  model(ETS(Exports ~ error(tipo) + trend(tipo) + season(tipo)))
```
...
Ese ajuste se guarda en un objeto de tipo modelo (`mdl_df`): para poder ver la info en un **formato tabulado "normal" basta con hacer `augment()`**: `.fitted` guarda las estimaciones, `.resid` los residuales estimados (y en este caso coincide con `.innov`)
...
Tras modelizar la serie podemos predecir de manera sencilla los valores con `forecast(h = n_instantes_futuros)` indicándole los **valores futuros que queremos predecir** (para convertirlo a un **formato normal**, dado que ya no es un modelo como antes, debemos convertirlo con `as_tsibble()`)
...
Podemos visualizar el ajuste (del pasado) haciendo `pivot_longer()`como siempre o directamente haciendo uso de `autoplot()` indicándole la variable a usar como línea base

```{r}
fit_algeria |>
  augment() |>
  autoplot(.vars = Exports) +
...
y la predicción (del futuro) con `autoplot()`
predict_algeria |>
