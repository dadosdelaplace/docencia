...
Con el paquete `{corrplot}` también podemos [**visualizar la matriz de correlaciones**]{.hl-yellow} de distintas formas (el tamaño y alpha indica su magnitud en valor absoluto, el color su signo)
...
Como ya hemos mencionado la correlación nos sirve para [**cuantificar el grado de asociación entre dos variables cuantitativas**]{.hl-yellow} pero siempre asumiendo una asociación muy particular: [**solo mide asociación LINEAL**]{.hl-yellow}

. . .

En matemáticas LINEAL = recta, así que en realidad la correlación nos está cuantificando [**cómo se agrupan nuestra nube de puntos a un recta**]{.hl-yellow} (la conocida como recta de regresión).
...
* $E \left[ \right]$ es lo que se conoce como **esperanza o media poblacional** (lo que puedes modelizar)

* $\varepsilon$ será el [**error o ruido**]{.hl-yellow}, una [**variable aleatoria de media 0**]{.hl-yellow} $E \left[\varepsilon | \boldsymbol{X} = x \right] = 0$ (el error debería ser reducido a **algo aleatorio (irreducible)**).
...
En el caso concreto de la [**regresión lineal univariante**]{.hl-yellow} nuestro modelo será una **simple recta**:

$$E \left[Y | \boldsymbol{X} = x \right]  = \beta_0 + \beta_1 X_1$$

. . .

Su estimación será por tanto

$$\widehat{Y} := \widehat{E \left[Y | \boldsymbol{X} = x \right]} = \widehat{f}(\mathbf{X}) = \widehat{\beta_0} + \widehat{\beta_1} X_1 $$

* El modelo $\widehat{f}(\cdot)$ será una [**recta (lineal)**]{.hl-yellow}

* El objetivo será obtener la estimación de los $\widehat{\beta}_0$ y $\widehat{\beta}_1$ tal que [**minimice lo que nos equivocamos**]{.hl-yellow}

---

## Lo que nos equivocamos

Una manera de cuantificar lo que nuestro modelo se equivoca es lo que llamamos [**varianza residual o error cuadrático medio**]{.hl-yellow} (varianza del error), que podemos estimar como sigue (ya que la [**media muestral de los errores será cero**]{.hl-purple})

$$s_{r}^{2} = \frac{1}{n} \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^{2} \quad \text{promedio de errores al cuadrado}$$
...
Simplemente necesitamos la función `predict(modelo, nuevos_datos)`, usando como argumentos el **modelo entrenado** y un [**nuevo dataset que contenga una/s columna/s llamadas igual que nuestras variables predictores**]{.hl-yellow} con los valores de la predictora `X` para los que queremos predecir


```{r}
predict(ajuste_lineal,
...
Podemos incluirlo todo en un tibble en el que tengamos los nuevos valores de `X` y las predicciones

```{r}
predicciones <- 
  tibble("height" = c(15, 81.3, 153.1, 189.3, 210.3),
         "mass_predict" = predict(ajuste_lineal, tibble(height)))
predicciones
```
...
## Fases de la regresión


Hasta ahora, dados unos datos, conocemos las siguientes fases del proceso:


1. [**Análisis exploratorio inicial**]{.hl-yellow} tanto numérico como visualizando. ¿Son numéricas? ¿Cómo se distribuyen? ¿Hay datos atípicos o ausentes?

2. [**Análisis de dependencia**]{.hl-yellow}. ¿Qué predictora correlaciona más con la objetivo $Y$? ¿Cómo se relacionan las predictoras entre sí?

3. [**Formulación**]{.hl-yellow} del modelo

4. [**Fase de estimación/predicción**]{.hl-yellow}. ¿Cuáles son los parámetros? ¿Cómo se interpretan?

---

## Caso práctico

Vamos a poner en práctica lo aprendido con el dataset `wine.csv`
...
# Clase 10: [regresión lineal]{.flow} {#reg-lineal}

[**Fases de una modelización lineal**]{style="color:#444442;"}
...
Pero hasta ahora no hemos pedido nada a la muestra $\left\lbrace \left( X_i, Y_i \right) \right\rbrace_{i=1}^{n}$, ¿para qué necesitaríamos [**hipótesis**]{.hl-yellow} entonces?

. . .

La razón es que, hasta ahora, lo único que hemos podido realizar es una [**estimación puntual (muestral)**]{.hl-yellow} de los parámetros, pero dado que dichos estimadores serán variables aleatorias, necesitaremos realizar [**inferencia estadística**]{.hl-yellow} (recuerda: los parámetros son simplemente estimaciones para esa muestra de la población).

. . .

Para poder cuantificar la [**variabilidad y precisión de nuestras estimaciones**]{.hl-yellow} necesitaremos que los datos cumplan ciertas [**hipótesis**]{.hl-purple}: lo interesante no es la estimación puntual de los parámetros a partir de la muestra sino lo que [**podamos inferir de ellos a la población**]{.hl-green}

---

## Fases de la regresión

Por ello en un análisis de regresión solemos tener 3 fases más:


5. [**Fase de diagnosis**]{.hl-yellow}. ¿Se cumplen las hipótesis para hacer inferencia?

6. [**Fase de inferencia**]{.hl-yellow}. ¿Qué variabilidad tiene la estimación? ¿Hay efecto lineal significativo? 

7. [**Fase de evaluación**]{.hl-yellow}:
  - ¿Es significativo el modelo completo? [**ANOVA: análisis de la varianza**]{.hl-purple}
  - ¿Qué % de información de la predictora explica el modelo? [**Parámetros de bondad de ajuste**]{.hl-purple} ($R^2$ por ejemplo)
  - Otras herramientas que podemos usar: por ejemplo [**enfrentar los valores ajustados con los valores reales**]{.hl-yellow}
...
1. [**Linealidad**]{.hl-green}: el valor esperado de $Y$ es modelizable con una recta $\widehat{Y}  = \beta_0 + \beta_1 x$

. . .

2. [**Homocedasticidad**]{.hl-green}: el modelo no podrá explicar toda la información (a veces se equivocará por arriba y otras por abajo), pero necesitamos que la [**varianza del error sea finita y constante**]{.hl-yellow}, tal que $\sigma_{r}^2 = \sigma_{\varepsilon}^2  = cte < \infty$, que no varíe según aumenta o disminuye la $X$.
...
4. [**Independencia**]{.hl-green}: los errores $\left\lbrace \varepsilon_i \right\rbrace_{i=1}^{n}$ deben ser independientes entre sí (el error en una observación no depende de otras). En particular, serán **incorrelados** ${\rm Cor}_{\varepsilon_i \varepsilon_j} = 0$ para todo $\quad i \neq j$
...
Para esta fase haremos uso de los paquetes `{performance}` y `{olsrr}`
...
#| eval: false
library(performance)
performance::check_model(ajuste_AGST)
```
...
Si te fijas [**ambos parámetros no son significativamente distintos de 0**]{.hl-green}: no presentan una tendencia en los residuos (lineal al menos)
...
```{r}
performance::check_heteroscedasticity(ajuste_AGST)
```
...
```{r}
performance::check_autocorrelation(ajuste_AGST)
```
...
```{r}
#| code-fold: true
ggplot(tibble("lag1" = lag(ajuste_AGST$residuals),
              "residuals" = ajuste_AGST$residuals),
...
Los $SE$ o [**standard errors**]{.hl-yellow} del modelo son
...
Pero tenemos un problema: la [**varianza residual poblacional**]{.hl-yellow} (varianza población del error) $\sigma_{\varepsilon}^{2}$ es [**desconocida**]{.hl-yellow}. Como suele pasar, la [**sustituiremos por un estimador (insesgado)**]{.hl-yellow} de la misma, donde $p$ es el número de variables (lo que R llama `std error` en la salida)

. . .

$$\widehat{\sigma_{\varepsilon}^{2}} =  \frac{1}{n-p-1} \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^2 =_{p=1}\frac{1}{n-2} \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^2$$
...
```{r}
ajuste_lineal <- lm(data = datos, formula = Price ~ AGST)
ajuste_lineal |> summary()
```
...
Si te fijas uno de los p-valores está por debajo de $\alpha = 0.05$ (**hay evidencias de que hay un efecto lineal significativo entre $X$ e $Y$**) pero el asociado al intercepto no: [**debemos eliminar dicho parámetro $\beta_0$**]{.hl-yellow} centrando la recta.

---

## Inferencia de los parámetros

Para ello basta con añadir `-1` al inicio de la fórmula.

```{r}
ajuste_AGST <- lm(data = datos, formula = Price ~ -1 + AGST)
ajuste_AGST |> summary()
```
...
```{r}
#| code-fold: true
ggplot(tibble("y" = ajuste_lineal$model$Price, "y_est" = ajuste_lineal$fitted.values),
       aes(x = y, y = y_est)) +
...
En el gráfica podemos ver como los [**valores reales vs estimados**]{.hl-green} están cercanos a la diagonal: el error cometido es muy pequeño.
...
Solo en el [**caso de la reg lineal**]{.hl-yellow} se puede demostra que 
...
```{r}
datos_extras <-
  tibble("y" = datos$y, "x1" = datos$x1, "x2" = rnorm(1000), "x3" = rnorm(1000),
         "x4" = rnorm(1000), "x5" = rnorm(1000), "x6" = rnorm(1000), "x7" = rnorm(1000),
         "x8" = rnorm(1000), "x9" = rnorm(1000), "x10" = rnorm(1000), "x11" = rnorm(1000),
         "x12" = rnorm(1000), "x13" = rnorm(1000), "x14" = rnorm(1000), "x15" = rnorm(1000),
         "x16" = rnorm(1000), "x17" = rnorm(1000), "x18" = rnorm(1000), "x19" = rnorm(1000),
         "x20" = rnorm(1000), "x21" = rnorm(1000), "x22" = rnorm(1000), "x23" = rnorm(1000))
ajuste_6pred <- lm(data = datos_extras, formula = y ~ x1 + x2 + x3 + x4 + x5 + x6)
ajuste_23pred <- lm(data = datos_extras, formula = y ~ .)
performance::compare_performance(ajuste_lineal, ajuste_6pred, ajuste_23pred)
```
