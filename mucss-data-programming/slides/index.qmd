...
Currently, [**Assistant Professor at UPM**]{.hl-yellow}. Disseminating via [**bluesky**](https://bsky.app/profile/dadosdelaplace.bsky.social) e [**Instagram**](https://instagram.com/javieralvarezliebana)
...
- [**Final exam**]{.hl-yellow} on 09/09/2025, 15:00-16:10 (70%). 
...
- [**Final exam**]{.hl-yellow} with all materials and internet (25%). [**You are exempt if you get more than 7/10**]{.hl-purple} in the previous tasks.
...
|:------:|:--------:|:--------:|:------:|:------:|:------:|:------:|:------:|:------:|:------:|
| [0-1](#clase-0-1) | S1 | 1 sep | First steps | [💻](#tu-turno-0-1-1)  |  |  | 
| [0-2](#clase-0-2) | S1 | 3 sep | First data: concatenate values and databases | [💻](#tu-turno-0-2-1) [💻](#tu-turno-0-2-2) [💻](#tu-turno-0-2-3) [💻](#tu-turno-0-2-4) |  [🐣](#caso-practico-0-2-1) [🐣](#caso-practico-0-2-2) |  |
| [0-3](#clase-0-3) | S1 | 8 sep | Quarto and if-else | [💻](#tu-turno-0-3-1) [💻](#tu-turno-0-3-2) [💻](#tu-turno-0-3-3) |  [🐣](#caso-practico-0-3-1) [🐣](#caso-practico-0-3-2) [🐣](#caso-practico-0-3-3) |  |
| [0-4](#clase-0-4) | S2 | 9 sep | Final task| | | [🎯 (70%)](#clase-0-4) |
:::
:::
...
| [7](#clase-7) | S8 | 24 oct | Import/export |  [💻](#tu-turno-7-1) | [🐣](#caso-practico-7-1)  |  |
| [8](#clase-8) | S10 | 6 nov | Purrr and forcats packages  | [💻](#tu-turno-8-1) [💻](#tu-turno-8-2)  |  [🐣](#caso-practico-8-1) |  |
...
* [💻 Solved exercises: type of variables](#tu-turno-0-2-1) 

* [💻 Solved exercises: vectors](#tu-turno-0-2-2) 

* [💻 Solved exercises: matrix](#tu-turno-0-2-3) 

* [💻 Solved exercises: data.frame and tibble](#tu-turno-0-2-4) 
...
[**Final task (70%)**: 09/09/2024 from 15:00 to 16:10]{style="color:#444442;"}
...
Evaluation just for the first 9 lessons (Javi's module).

-  [**Attendance and individual participation**]{.hl-yellow} (10%)

. . .

- [**2 individual tasks**]{.hl-yellow} sent during the course (20%-30%). Deadlines: 07/10/2025 and 25/10/2025.

. . .

- [**Final exam**]{.hl-yellow} with all materials and internet (40%, 11/11/2025). [**You are exempt if you get more than 7.5/10**]{.hl-purple} in the previous tasks. [**You must get more than 3/10**]{.hl-purple} to be considered.

. . .

* [**Max grade**]{.hl-yellow}: to get a grade greater than 9/10 you should get at least 9/10 in the average of individual tasks.
...
* [📊 Data](https://drive.google.com/drive/folders/18Ok6Epqcimszqguj_JTTLuSbs5Ot5Srd?usp=sharing)
...
## 🐣 Case study I: CIS feminism {#caso-practico-8-1}
...
Try to answer the questions posed in the [**workbook**](https://javieralvarezliebana.quarto.pub/joins-import-mucss/#case-study-ii-cis-survey)
...
[**GitHub**]{.hl-yellow} is the most well-known collaborative platform based on the [**Git version control system**]{.hl-yellow}

. . .

-  [**What is Git?**]{.hl-purple} Git is a [**version control**]{.hl-yellow} system: a kind of [**Dropbox**]{.hl-yellow} to facilitate [**collaborative programming**]{.hl-yellow} among a group of people, allowing to keep track of the [**traceability of changes**]{.hl-yellow} made.

. . .

- [**What is Github?**]{.hl-purple} Our [**platform/interface**]{.hl-yellow} for running version control: it will serve us not only to work collaboratively but to [**make transparent**]{.hl-yellow} the process of building our code projects.
...
After making an account in Github, [**up right**]{.hl-purple} we will have a circle, and clicking on [**Your Profile**]{.hl-purple}, we will see something like this
...
::: {.column width="45%"}
- [**Edit profile**]{.hl-purple}: allows us to add a [**description and profile picture**]{.hl-yellow}.

- [**Overview**]{.hl-purple}: in that squares panel will [**view our activity**]{.hl-yellow} over time.

- [**Repositories**]{.hl-purple}: the code will be uploaded to [**repositories**]{.hl-yellow}, the equivalent of our shared folders in Dropbox.
:::
:::
...
- [**Access code**]{.hl-purple} from others

- [**Propose improvements**]{.hl-purple} to other users, and even propose [**bug fixes we detect**]{.hl-yellow} for software we use.
...
Package code that is not uploaded to CRAN can be [**installed as code from Github**]{.hl-yellow}

---

## Installing from Github

For example, let's install a package called [`{peRReo}`](https://github.com/jbgb13/peRReo), whose only function is to give us [**color palettes**]{.hl-yellow} based on [**urban music albums**]{.hl-yellow} covers.
...
To do this we first need to install a [**developer package set**]{.hl-yellow} called `{devtools}`, which will allow us to install from Github
...
The [**installation instructions**]{.hl-yellow} are usually detailed in the repository cover page (`README.md` file)
...
In most cases the `install_github()` function (from the package we just installed) will suffice, passing it the [**repository path**]{.hl-yellow} (without “github.com/”) as an argument.
...
Most of the time what we upload will not be an R package as such but [**we will upload a more or less organized**]{.hl-yellow} and commented code. In that case we can [**download the whole repo**]{.hl-yellow} by clicking [**Code**]{.hl-green} and then Download ZIP.

For example, let's download the dataviz scripts that have been uploaded from the [Centre d'Estudis d'Opinió](https://github.com/ceopinio/bop-grafics) (Catalan CIS).
...
Let's [**create our first repository**]{.hl-yellow} that will also serve as the [**cover page**]{.hl-yellow} of our Github profile.

1. [**Repositories**]{.hl-purple}: click on the Repositories tab.

2. [**New**]{.hl-purple}: click on the [**New**]{.hl-green} button to create a new repository.
...
- [**Repository name**]{.hl-purple}: the [**repository name**]{.hl-yellow}. In this case we are going to create a very specific repository: the name must [**exactly match your username**]{.hl-yellow}

- [**Description**]{.hl-purple}: description of your repository. In this case it will be a presentation repo.
![](img/repo_init_1.png)
...
- [**Public vs private**]{.hl-purple}: with each repository we will have the option to make the repository

  - [**public**]{.hl-purple}: all users will be able to see the code as well as the traceability of your development (what is added and when). It is for me the most recommended option when you want to give [**visibility and transparency**]{.hl-yellow} to your work
  - [**private**]{.hl-purple}: only those users you allow will have access to the repository. Nothing can be viewed or installed from it outside of Github.

![](img/repo_init_2.png)

In this particular case, since it will be a presentation repository, we will [**make it public**]{.hl-yellow}.

---

## First repo

- **Add a README file**]{.hl-purple}: a README file will be the file where we will include the [**instructions and usage details**]{.hl-yellow} to the others (in the case of `{peRReo}` it was the file containing the installation details).

![](img/repo_init_3.png)

For the time being [**ignore other fields**]{.hl-net} for this first repository.
...
By default Github assumes that this repository, with the [**same name as our user**]{.hl-yellow} will be the repository we want to be presented at startup when someone logs into our profile, and will be the repository where [**include in the README.md**]{.hl-yellow} a presentation of us and an index of your work (if you want).
...
Notice that now in our [**cover page we have said README.md**]{.hl-yellow} that we can customize to our liking using [**html and markdown**]{.hl-yellow}.Here you can see [**some examples of README.md**](https://github.com/matiassingers/awesome-readme)

---

## Code repo

Once we have our presentation README (remember you can [**customize to your liking with html and markdown**]{.hl-yellow}) let's create a [**code repository**]{.hl-yellow}.

. . .

If it was already important to [**work with projects**]{.hl-yellow} in `RStudio`, when combined with Github it is even more crucial that we create a project before uploading code, so let's create a test one called `repo-github-1`.

. . .

In this project we are going to [**create a script**]{.hl-yellow} (in my case called **codigo.R**) in which you must do the following steps:

---

## Code repo

1.  [**Download**]{.hl-yellow} directly from the [ISCIII](https://cnecovid.isciii.es/covid19/resources) page the file named `casos_hosp_hosp_uci_def_def_sexo_edad_provres.csv`.
...
2. [**Filter**]{.hl-yellow} data for Madrid (`“M”`), between March 1, 2020 and December 31, 2020, and with sex `“H”`. After that keep the columns `fecha`, `grupo_edad`, `num_casos` (that order). Finally get the sum of daily cases per date (regardless of age group).
...
datos_madrid <-
  ...
  filter(provincia_iso == "M" &
           between(fecha, as_date("2020-03-01"), as_date("2020-12-31")) &
           sexo == "H") |> 
  select(fecha, grupo_edad, num_casos) |> 
  summarise(num_casos = sum(num_casos), .by = c(fecha))
```
...
3. [**Export the dataset to a csv**]{.hl-yellow} in a folder named `exported`.
...
4. Create a [**line graph**]{.hl-yellow} that has on the x-axis date, on the y-axis cases, with one curve per sex (separate graphs).
...
grafica <-
  ggplot(datos_madrid) +
  geom_col(aes(x = fecha, y = num_casos),
...
5. After that [**export the graph**]{.hl-yellow}

```{r}
#| eval: false
ggsave(filename = "./exportado/ggplot.png", plot = grafica,
...
How do we [**upload the project**]{.hl-yellow}? Let's go back to [**create a project from scratch**]{.hl-yellow}. We didn't talk about two important fields earlier:
...
::: {.column width="50%"}
- Add .gitignore` will allow us to [**select the language**]{.hl-yellow} our project will be in so that Github understands it when syncing (and doesn't update things it shouldn't).

- `Choose a license` will allow us to [**select the license**]{.hl-yellow} which will determine the conditions under which others will be able to reuse your code.
:::
:::
...
To upload the files we will click on [**Add file \< Upload File**]{.hl-purple} and [**drag ALL files**]{.hl-yellow} from our project folder.

---

## Code repo

After uploading files we will have a box called [**Commit changes**]{.hl-purple}

![](./img/commit-1.png)

A [**commit**]{.hl-purple} is a [**modification of the repository**]{.hl-yellow} with something to be added/removed/modified, and it is advisable to use this box to [**summarize what the modification**]{.hl-yellow} is about, so that the change is traced.

---

## Code repo

By clicking on the clock where it indicates the [**number of commits**]{.hl-yellow} we access the [**history of commits (changes)**]{.hl-yellow} with time, day, author, comments, etc.
...
Let's make a [**change in our code**]{.hl-yellow}: in your local code (local --> your computer), instead of filtering by Madrid do the [**filter by Barcelona**]{.hl-yellow}, save the code and upload the new file in the repository (with the same name, Github will do the overwrite)
...
::: columns
::: {.column width="40%"}
If we now [**consult the commit**]{.hl-yellow}, next to it there is a number that identifies it, and clicking on it summarizes the changes: not only [**stores all past versions**]{.hl-yellow} but also [**shows the differences between the changed files**]{.hl-yellow}
:::
...
::: {.column width="28%"}
We have [**two display modes**]{.hl-yellow} of the changes: the split mode shows us the old and the new, with the inclusions in green and what is no longer there in red; and the unified mode shows us everything in the same document.
:::
:::
...
Github even allows us to [**retrieve a past version**]{.hl-yellow} from our repository by clicking on the third commit icon.
...
If you notice now next to `1 branch` we have a [**drop-down menu**]{.hl-yellow} where before it used to say `main` and now a commit identifier number. We will talk about the idea of [**branch (branch)**]{.hl-yellow} later.
...
1. Create a new repository in Github (called `repo-github-2`) where you will have hosted your R project.


2. Create a project in `RStudio` called (for example) `project-qmd`

3. Once inside the project in `RStudio` click on `File < New File < Quarto Document`.
:::
...
You should have a document similar to this: a [**fourth markdown (.qmd)**]{.hl-yellow}, a document that will allow us to include [**markdown + code**]{.hl-yellow} (it can be `R` or it can be `Observable`, `D3`, etc).
...
- [**Working in a team**]{.hl-yellow} building a draft of a piece.

- Taking [**notes or reports**]{.hl-yellow} for yourself.

- [**Presenting**]{.hl-yellow} your work to your peers.
...
How to [**convert our repository properly into a web**]{.hl-yellow}?
...
3. In the subsection `branch` select the only branch that we have now (`main`)

4. Select the folder where you have the `.html` (in complex web it will be as in any web in `docs`, in something simple it will be in the root path of the repository)
...
If you look at the [**top of the repository**]{.hl-yellow} now we have an orange icon, which indicates that the [**web is in the process of being deployed**]{.hl-orange}
...
After a few seconds (depending on the size of the website and your internet connection) that [**icon will change to a green check**]{.hl-green}

![](./img/github-verde.png)

The [**default web link**]{.hl-yellow} will be `{username}.github.io/{repo_name}`.
...
Wait a minute! Right now our website [**is not showing us our .qmd**]{.hl-red}, but by default the [**README.md**]{.hl-yellow}.

. . .

For Github to understand that we want to display that `.html` that we have generated from the `.qmd` we go in our local project to [**delete**]{.hl-yellow} everything that is not our `.Rproj` file and our `.qmd` file, and we go to [**name**]{.hl-yellow} the latter calling it `index.qmd`, and we recompile it to [**generate an index.html**]{.hl-yellow}

---

## Github pages

Let's [**upload to Github that new project**]{.hl-yellow} with the name change (called `repo-github-3`) to see later the differences between one and the other one.
...
If we [**repeat the process to make a Page**]{.hl-yellow} and wait for the green tick...
...
::: {.column width="50%"}
If your `.qmd` is already called `index.qmd`, automatically, when Github detects an `index.html`, it interprets that [**index.html file**]{.hl-yellow} is the one that defines the web (and you can customize by adding a `css` style file)

[**We have a web**]{.hl-green} by simply clicking on Pages :)
:::
:::
...
::: {.column width="55%"}
The way to write will be the same as a normal `.qmd` only now [**each slide will be separated**]{.hl-yellow} with a `---` (using style files we can customize whatever we want)

Call the file directly `index.qmd`, upload it to Github and with one click in Pages you have a [**web with your slides**]{.hl-yellow}
:::
:::
