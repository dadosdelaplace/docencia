...
Before we continue, it’s important to know something as soon as possible: [**starting with programming can be frustrating**]{.hl-yellow}. Just like when learning a new language, the first obstacle is not so much what to say but how to say it correctly. The same goes for `R`, so let’s [**standardize our programming style**]{.hl-yellow} as much as possible to avoid future errors.
