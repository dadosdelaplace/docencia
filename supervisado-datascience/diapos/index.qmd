...
| [3](#clase-3) | S2 | 30 ene | Análisis de la varianza | [💻](#tu-turno-3-1)  | [🐣](#caso-practico-3-1) [🐣](#caso-practico-3-2) |  | 
| [4](#clase-4) | S3 | 4 feb | Análisis de la varianza | ... |  |  | 
...
# Clase 3: análisis de la varianza {#clase-3}

[**Asociación cuali vs cuanti: análisis de la varianza  (30 de enero de 2025)**]{style="color:#444442;"}
...
¿Cómo [**conceptualizar matemáticamente un modelo que asuma que todas las asignaturas tienen medias similares**]{.hl-yellow}?
...
donde [**$\mu_j$ representa la media de los estudiantes en la asignatura $j$**]{.hl-yellow} y la [**perturbación $\varepsilon_{ij}$ representa la desviación**]{.hl-purple} de la nota en la asignatura $j$ del estudiante $i$ (representan la variabilidad intrínseca del EXPERIMENTO).
...
2. [**Varianza constante (homocedasticidad)**]{.hl-green}: $V[\varepsilon_{ij}] = cte = \sigma^2$ para todo $i,j$ (las desviaciones respecto a la media de la asignatura son iguales)
...
Los [**esimadores insesgados**]{.hl-yellow} de dichos parámetros poblacionales $\left(\mu_j, \sigma^2 \right)$ son

* [**Media**]{.hl-yellow}: $\hat{\mu}_{j} =  \overline{y}_j$, lo que implica que $\hat{\varepsilon}_{ij} = y_{ij} - \overline{y}_j$.
...
Sin embargo, dado que cada $\sum_{i=1}^{n_j} \left(\frac{y_{ij} - \overline{y}_j}{\sigma} \right)^2$ son **independientes** y siguen una $\chi^{2}_{n_j - 1}$ (suma de normales - tipificadas - al cuadrado), tenemos que
...
$\hat{\sigma}^2$ [**no es un estimador insesgado**]{.hl-red} de $\sigma^2$ así que usaremos la conocida como [**varianza residual**]{.hl-yellow}
...
Fíjate que dado que $\hat{\mu}_j = \overline{y}_j$, hemos llamado

$$\hat{\varepsilon}_{ij} = y_{ij} - \overline{y}_j \sim N \left(\hat{\mu}_j, \hat{s}_{r}^2 \right)$$
 a los [**residuos estimados**]{.hl-yellow} (valor real - estimación)
...
La  [**hipótesis nula**]{.hl-green} es que las [**diferencias entre notas en las distintas asignaturas**]{.hl-yellow} se debe únicamente a una variabilidad aleatoria (y no una diferencia sistemática).

. . .

El concepto «diferencias» se traduce en **variabilidad**, es decir, la [**hipótesis nula puede reformularse**]{.hl-green} como

$$H_0:~\text{variab. notas de distintas asignaturas es pequeña} = \text{variab. medias es pequeña}$$
...
Hemos reformulado antes la hipótesis nula como

$$H_0:~\text{variab. notas de distintas asignaturas es pequeña} = \text{variab. medias es pequeña}$$

¿Cómo cuantificar si esa [**variabilidad entre las medias**]{.hl-yellow} (varianza ENTRE grupos, debida a estar en un grupo u otro) es pequeña?

. . .

La vamos a [**comparar respecto a la variabilidad promedio (intrínseca) de los datos**]{.hl-purple} (promedio ponderado de la varianza DENTRO de los grupos, no explicada por los grupos sino intrínseca a los datos)
...
¿Cómo calcular la [**varianza explicada**]{.hl-red} por la variable de grupo (la variabilidad de las medias de cada grupo) y la [**varianza no explicada**]{.hl-purple} por el grupo (el promedio de las varianzas dentro de cada grupo, intrínseca a los datos)?
...
El [**segundo término**]{.hl-red} es en realidad la [**variabilidad (ponderada) de las medias de los grupos**]{.hl-red} (cómo fluctua la media entre los grupos)
...
Es la [**variabilidad explicada por los grupos**]{.hl-red} (variabilidad - de la media - ENTRE grupos) y [**cuantifica la variabilidad de las medias**]{.hl-red}. ¿Es mucho? ¿Poco? La vamos a enfrentar a la variabilidad no explicada por los grupos (inherente a la variable)
...
El [**primer término**]{.hl-purple} es en realidad el [**promedio de la variabilidad $s_{j}^2$ DENTRO de cada grupo**]{.hl-purple} (el promedio de la variabilidad intragrupo)
...
que se relaciona con la [**varianza residual (estimada)**]{.hl-purple} como $\hat{\sigma}^2 = \frac{n-c}{n} \hat{s}_{r}^{2}$.


Es la [**variabilidad NO explicada (residual) por el factor**]{.hl-purple} (promedio - de manera ponderada - de la variabilidad DENTRO de cada grupo) y [**cuantifica la variabilidad inherente a los datos (promediando las variabilidades de cada grupo)**]{.hl-red}

---

## ANOVA: un factor

Dado que el modelo es lineal, la [**variabilidad o información total**]{.hl-yellow} se puede descomponer como [**varianza explicada**]{.hl-red} (VE o suma cuadrados explicada - SSE) más [**varianza no explicada**]{.hl-purple} (VR o suma cuadrados residuales - SSR)
...
Así el objetivo será construir un **estadístico** para cuantificar si $VE$ (variabilidad entre las medias) es mucho más grande que $VR$ (variabilidad basal, la variabilidad inherente de nuestra variable)
...
## 🐣 Caso práctico I: pendiente {#caso-practico-4-1}

---

## 🐣 Caso práctico II: pendiente {#caso-practico-4-2}
