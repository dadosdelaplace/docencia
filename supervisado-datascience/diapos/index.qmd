...
$$\widehat{\sigma_{\varepsilon}^{2}} = \frac{n}{n-p-1} s_{r}^{2} = \frac{1}{n-p-1} \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^2 =_{p=1} \frac{1}{n-2} \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^2 := \sqrt{\text{Res standard error}}$$
...
# Clase 8: predicción, simulación y caso práctico {#clase-8}
...
* [**Ejercicio 8**]{.hl-yellow}: con lo obtenido calcula sin hacer uso de `lm()` las estimaciones de las desviaciones típicas de las variables aleatorias $\widehat{\beta}_0$ y  $\widehat{\beta}_1$ que representan nuestros estimadores. Genera dos variables aleatorias ($n = 5000$) con la distribución de ambas variables
