...
📝 ¿Cuál es la ecuación del modelo? ¿Son los parámetros significativos? ¿Por qué? Interpreta el valor de ambos parámetros de manera adecuada.
...
## 🐣 Caso práctico I: precisión vs sesgo {#caso-practico-8-1}
...
# Clase 11: caso práctico {#clase-11}
...
1. Simula unos datos ($n = 500$) bajo la hipótesis (junta $y$ y $x$ en la misma tabla)
...
2. Ajusta un modelo de regresión lineal univariante e interpreta todo lo que consideres (asume que la diagnosis se cumple ya que lo hemos simulado para que así sea) 
...
3. Diseña un bucle de 490 iteraciones de manera que, en cada iteración, incorpores a la tabla una nueva predictora $X_i \sim N(0, 1)$. Fíjate que las [**predictoras nuevas no tienen ningún tipo de relación con $y$**]{.hl-red} (son "basura" para el modelo). En cada iteración haz un ajuste de $y$ vs todas y guarda su $R2$
...
4. Crea una tabla con dos columnas: iteración y R2. Tras ello visualiza la evolución del R2 en función del número de la iteración (=número de predictoras basura)
...
[**Moraleja**]{.hl-yellow}: a mayor nº de predictoras, más se infla $R^2$ (¡aunque sean basura!)
...
5. Simula una colección de datos ($n = 500$) bajo la hipótesis de que $Y = -1 + 3*X_1 + \varepsilon$ con $X \sim N(-3, \sigma = 1.2)$ y
...
6. Realiza los 6 ajustes, uno para cada dataset, siempre $y$ vs $x$. [**IMPORTANTE**]{.hl-yellow}: fíjate que la parte a explicar, la modelizable, la parte no aleatoria, ¡es siempre la misma! Así que el modelo debería ser parecido siempre. Usa `compare_performance()` para compararlos.
...
**¡La supuesta calidad del ajuste desciende drásticamente, siendo igual de bueno!** La parte que se puede predecir está perfectamente ajustada pero tenemos más ruido en los datos (ruido = no modelizable)

---

## Simulación

7. Y si tenemos un modelo con un alto $R^2$, [**¿no hace falta que cumpla las hipótesis?**]{.hl-yellow}. Basándote en  $X \sim N(-3, \sigma = 1.5)$ y $\varepsilon \sim N(0, \sigma = 1)$  con $n = 500$, simula un modelo entre $X$ e $Y$ que incumpla la hipótesis de linealidad.
...
8. Realiza el ajuste lineal y visualiza residuos vs valores predichos.
...
Nos queda un patrón cuadrático porque **en los datos existe dicho patrón** y no lo hemos incluido en el modelo (hipótesis de linealidad errónea)

---

## Simulación

Fíjate que [**tenemos un $R^2$ bastante alto**]{.hl-yellow} pero no tiene sentido su interpretación ya que la recta como modelo aquí no tiene sentido
...
9. Simula los datos bajo la siguiente hipótesis ($x_i = 0.01 + 0.01*(i-1)$) 
...
10. Realiza el ajuste de $y ~ x$ y observa el $R^2$
...
11.  Fíjate que [**tenemos un $R^2$ bastante alto**]{.hl-yellow} pero... ¿tiene sentido su interpretación? Pinta en un diagrama de puntos + recta de regresión los datos $x$ vs $y$.
...
Aunque el $R^2$ es bastante alto, el modelo no tiene sentido, dando [**estimaciones cada vez más erradas**]{.hl-red}



# Clase 12: casos reales {#clase-12}

[**Regresión univariante con casos reales**]{style="color:#444442;"}
